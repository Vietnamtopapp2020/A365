...
@Component({
  ...
  templateUrl: './administrative-regions.page.html',
  styleUrls: ['./administrative-regions.page.scss'],
})
export class AdministrativeRegionsPage implements OnInit {
  listethnic = [
    ...
    if (this.sltProfiles == null) {
      ...
      if(this.txtname == ""){
        ...
        if (this.netWork.type.toUpperCase() != "NONE") {
          this.service.postAPIquestionmchatR(this.service.getHost() + 'cdc/testTakers', body).subscribe
            (rs => {
              ...
              this.router.navigate(['/ketquacdc'], navigationExtras);
            }, error => {
              var er = error.json();
      ...
      if (this.netWork.type.toUpperCase() != "NONE") {
        ...
        this.service.PutAPIanswersheetMchartR(this.service.getHost() + 'cdc/testTakers', body1).subscribe
          (rs => {
            let navigationExtras: NavigationExtras = {
              queryParams: {
                createdId: this.Data.createdId,
