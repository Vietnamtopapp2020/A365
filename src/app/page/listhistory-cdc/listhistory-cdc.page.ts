...
@Component({
  selector: 'app-listhistory-cdc',
  templateUrl: './listhistory-cdc.page.html',
  styleUrls: ['./listhistory-cdc.page.scss'],
})
export class ListhistoryCDCPage implements OnInit {
  childrenlist = [];
  kqasq: any[];
  answers = [];
  answersheetsid: any[];
  ...
  GetpostCDC() {
    if (this.netWork.type.toUpperCase() != "NONE") {
      this.service.postGet(this.service.getHost() + 'cdc/answersheets?skip=' + this.skip + '&take=' + this.take + '&query=' + this.query).subscribe
        (rs => {
...
if (this.service.CheckLoading) {
  ...
  CDCpost(data) {
    if (data.isCompleted) {
      ...
      this.router.navigate(['/update-cdc'], navigationExtras);
    }
  }
  backPage() {
