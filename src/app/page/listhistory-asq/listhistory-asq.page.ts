...
@Component({
  selector: 'app-listhistory-asq',
  templateUrl: './listhistory-asq.page.html',
  styleUrls: ['./listhistory-asq.page.scss'],
})
export class ListhistoryASQPage implements OnInit {
  childrenlist = [];
  kqasq: any[];
  answersheetsid: any[];
  getlistquestion: any[];
  ...
  GetpostASQ() {
    if (this.netWork.type.toUpperCase() != "NONE") {
      this.service.postGet(this.service.getHost() + 'asq/answersheets?skip=' + this.skip + '&take=' + this.take + '&query=' + this.query).subscribe
        (rs => {
          var result = rs.json();
...
if (this.service.CheckLoading) {
  ...
  ASQpost(data) {
    ...
    if (data.isCompleted) {
      ...
      this.router.navigate(['/update-asq'], navigationExtras);
    }
