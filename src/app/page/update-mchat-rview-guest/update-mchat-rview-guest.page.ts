...
@Component({
  selector: 'app-update-mchat-rview-guest',
  templateUrl: './update-mchat-rview-guest.page.html',
  styleUrls: ['./update-mchat-rview-guest.page.scss'],
})
export class UpdateMchatRViewGuestPage implements OnInit {
  ...
  GetResutlmcharR(data) {
    if (this.netWork.type.toUpperCase() != "NONE") {
      this.service.postGet(this.service.getHost() + 'mchatr/guest/answersheets/' + data.createdId + "?token=" + data.token).subscribe
        (rs => {
