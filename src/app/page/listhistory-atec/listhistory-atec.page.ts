...
@Component({
  selector: 'app-listhistory-atec',
  templateUrl: './listhistory-atec.page.html',
  styleUrls: ['./listhistory-atec.page.scss'],
})
export class ListhistoryATECPage implements OnInit {
  childrenlist = [];
  kqasq: any[];
  answersheetsid: any[];
  getlistquestion: any[];
  ...
  GetpostATEC() {
    if (this.netWork.type.toUpperCase() != "NONE") {
      this.service.postGet(this.service.getHost() + 'atec/answersheets?skip=' + this.skip + '&take=' + this.take + '&query=' + this.query).subscribe
        (rs => {
...
if (this.service.CheckLoading) {
  ...
  ATECpost(data) {
    if (data.isCompleted) {
      ...
      this.router.navigate(['/update-atec'], navigationExtras);
    }
  }
  backPage() {
