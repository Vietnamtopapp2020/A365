...
@Component({
  selector: 'app-createchildrenguestmchatr',
  templateUrl: './createchildrenguestmchatr.page.html',
  styleUrls: ['./createchildrenguestmchatr.page.scss'],
})
export class CreatechildrenguestmchatrPage implements OnInit {
  childrenlist = [];
  txtname = "";
  txtdob = "";
  sltGender = null;
  ...
  Creatchildren() {
    if (this.txtpregnantWeeks < 21 || this.txtpregnantWeeks > 45) {
      ...
      if (this.netWork.type.toUpperCase() != "NONE") {
        this.service.postAPIquestionmchatR(this.service.getHost() + 'mchatr/guest/session', body).subscribe
          (result => {
