...
@Component({
  selector: 'app-administrative-regions-asq',
  templateUrl: './administrative-regions-asq.page.html',
  styleUrls: ['./administrative-regions-asq.page.scss'],
})
export class AdministrativeRegionsASQPage implements OnInit {
  listethnic = [
  ...
  Creatadministrativeasq() {
    if (this.sltProfiles == null) {
      ...
      if (this.txtname == "") {
        this.service.message("Vui lòng nhập đầy đủ thông tin");
          ...
          if (this.netWork.type.toUpperCase() != "NONE") {
            this.service.postAPIquestionmchatR(this.service.getHost() + 'asq/testTakers', body).subscribe
              (rs => {
      ...
      if (this.netWork.type.toUpperCase() != "NONE") {
        this.service.PutAPIanswersheetMchartR(this.service.getHost() + 'asq/testTakers', body1).subscribe
          (rs => {
  ...
  relatedProfiles() {
    if (this.netWork.type.toUpperCase() != "NONE") {
      this.service.Getchildren(this.service.getHost() + 'children/' + this.Data.childId + '/relatedProfiles').subscribe
        (rs => {
          var result = rs.json();
