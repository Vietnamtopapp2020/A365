...
@Component({
  selector: 'app-listchildrenmchatrf',
  templateUrl: './listchildrenmchatrf.page.html',
  styleUrls: ['./listchildrenmchatrf.page.scss'],
})
export class ListchildrenmchatrfPage {
  asqDetails = [];
  asqDetails1 = [];
  childrenlist = [];
  getlistquestion = [];
...
if (this.service.CheckLoading) {
  ...
  MchartRF(data) {
    let navigationExtras: NavigationExtras = {
      queryParams: {
        ObjectPage: data,
        pageKQ: '/listchildrenmchatrf'
      }
    };
    if (data.isEligibleForMchatr == true) {
  ...
  Historymchartrf() {
    this.router.navigate(['/historymchartrf']);
  }
  backPage() {
    this.navCtrl.pop();
