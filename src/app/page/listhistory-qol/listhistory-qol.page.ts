...
@Component({
  selector: 'app-listhistory-qol',
  templateUrl: './listhistory-qol.page.html',
  styleUrls: ['./listhistory-qol.page.scss'],
})
export class ListhistoryQOLPage implements OnInit {
  childrenlist = [];
  kqasq: any[];
  answersheetsid: any[];
  getlistquestion: any[];
  ...
  GetpostQOL() {
    if (this.netWork.type.toUpperCase() != "NONE") {
      this.service.postGet(this.service.getHost() + 'qol/answersheets?skip=' + this.skip + '&take=' + this.take + '&query=' + this.query).subscribe
        (rs => {
          var result = rs.json();
...
if (this.service.CheckLoading) {
  ...
  QOLpost(data) {
    if (data.isCompleted) {
      ...
      this.router.navigate(['/update-qol'], navigationExtras);
    }
  }
